...
SEQUENCE41_REPORT_ROOT = Path("reports/sequence41/replay_eval")
...
def evaluate_pm_crypto_updown_sequence41_replay(
    *,
    fixture_root: str | Path,
    output_root: str | Path = ".",
    real_cached_artifact_roots: list[str | Path] | None = None,
    ...
    return _evaluate_from_progress(progress, sequence="41")


def write_pm_crypto_updown_sequence41_replay_eval_report(
    *,
    fixture_root: str | Path,
    output_root: str | Path = ".",
    real_cached_artifact_roots: list[str | Path] | None = None,
) -> dict[str, Any]:
    payload = evaluate_pm_crypto_updown_sequence41_replay(
        fixture_root=fixture_root,
        output_root=output_root,
        real_cached_artifact_roots=real_cached_artifact_roots,
    )
    payload["report_paths"] = _write_report(
        ...
        json_filename="latest_pm_crypto_updown_replay_eval.json",
        markdown_filename="latest_pm_crypto_updown_replay_eval.md",
        title="Sequence 41 PM Crypto UP/DOWN Replay Evaluation",
    )
    return payload


def _evaluate_from_progress(progress: dict[str, Any], *, sequence: str) -> dict[str, Any]:
    dataset = progress["dataset_report"]
    rows = dataset["rows"]
    primary_rows = dataset["primary_rows"]
    fixture_rows = [row for row in rows if row["source_quality"] == "fixture_real_shaped"]
    ...
    return {
        "schema_version": (
            "pm_crypto_updown_replay_eval_v1"
            if sequence == "41"
            else "pm_crypto_updown_real_cached_replay_eval_v1"
        ),
        "sequence": sequence,
        "candidate_id": CANDIDATE_ID,
        "evaluation_status": (
...
def _write_report(
    ...
    *,
    ...
    json_filename: str = "latest_real_cached_replay_eval.json",
    markdown_filename: str = "latest_real_cached_replay_eval.md",
    title: str = "Sequence 39 Real-Cached Replay Evaluation",
) -> dict[str, str]:
    root = Path(output_root) / report_root
    root.mkdir(parents=True, exist_ok=True)
    json_path = root / json_filename
    md_path = root / markdown_filename
