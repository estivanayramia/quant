...
from quant_os.readiness.autonomy_milestones import (
    ...
    build_sequence51_autonomy_milestones,
)
...
SEQUENCE51_REPORT_ROOT = Path("reports/sequence51/autonomy_milestones")
...
def write_sequence43_autonomy_milestone_report(
    *,
    bounded_shadow_readiness: dict[str, Any],
    output_root: str | Path = ".",
) -> dict[str, Any]:
    payload = build_sequence43_autonomy_milestones(
        bounded_shadow_readiness=bounded_shadow_readiness,
    )
    payload["report_paths"] = _write_report(
        payload,
        output_root=output_root,
        report_root=SEQUENCE43_REPORT_ROOT,
    )
    return payload


def write_sequence44_autonomy_milestone_report(
    *,
...
def write_sequence47_autonomy_milestone_report(
    *,
    candidate_readiness: dict[str, Any],
    output_root: str | Path = ".",
) -> dict[str, Any]:
    payload = build_sequence47_autonomy_milestones(
...
def write_sequence51_autonomy_milestone_report(
    *,
    weather_profit_readiness: dict[str, Any],
    output_root: str | Path = ".",
) -> dict[str, Any]:
    payload = build_sequence51_autonomy_milestones(
        weather_profit_readiness=weather_profit_readiness,
    )
    payload["report_paths"] = _write_report(
        payload,
        output_root=output_root,
        report_root=SEQUENCE51_REPORT_ROOT,
    )
    return payload
