...
REPORT_ROOT = Path("reports/sequence51/weather_paper_profit_readiness")

ALLOWED_PHASE51_STATUSES = [
...
def evaluate_weather_market_paper_profit_readiness(
    *,
    ...
    dataset_payload = dataset_payload or {
        "dataset_status": "WEATHER_DATA_CAPTURE_BLOCKED",
        "rows": [],
        "real_public_row_count": 0,
        "proof_row_count": 0,
        "fixture_row_count": 0,
        "blockers": ["NO_REAL_PUBLIC_WEATHER_MARKET_CAPTURE"],
    }
    paper_payload = paper_payload or run_weather_market_real_paper_proving(
    ...
    payload = {
        "schema_version": "weather_market_paper_profit_readiness_v1",
        "sequence": "51",
        ...
        "allowed_statuses": ALLOWED_PHASE51_STATUSES,
...
def write_weather_market_paper_profit_readiness_report(
    *,
    output_root: str | Path = ".",
    dataset_payload: dict[str, Any] | None = None,
    paper_payload: dict[str, Any] | None = None,
    ...
    if dataset_payload is None:
        ...
        dataset_payload = write_weather_market_dataset_report(output_root=output_root)
    if paper_payload is None:
        paper_payload = run_weather_market_real_paper_proving(
            dataset_payload.get("rows", []),
            output_root=output_root,
        )
    return evaluate_weather_market_paper_profit_readiness(
        dataset_payload=dataset_payload,
        paper_payload=paper_payload,
        output_root=output_root,
    )
...
def _blockers(*, dataset_payload: dict[str, Any], paper_payload: dict[str, Any]) -> list[str]:
    ...
    if not fixture_rows:
        fixture_rows = sum(1 for row in dataset_payload.get("rows", []) if row.get("fixture_only"))
    ...
    if dataset_payload.get("proof_row_count", 0) == 0 and dataset_payload.get(
        "real_public_row_count",
        0,
    ):
        blockers.add("RESOLUTION_LABELS_MISSING")
    if dataset_payload.get("proof_row_count", 0) and dataset_payload.get("proof_row_count", 0) < 30:
...
def _readiness_status(
    *,
    ...
    if dataset_payload.get("dataset_status") == "MARKET_DATA_CAPTURE_BLOCKED":
        return "MARKET_DATA_CAPTURE_BLOCKED"
    if dataset_payload.get("dataset_status") == "WEATHER_DATA_CAPTURE_BLOCKED":
        return "WEATHER_DATA_CAPTURE_BLOCKED"
...
def _autonomy_milestones(
    *,
    ...
    return {
        ...
        "weather_capture_plan": "met",
        "real_weather_data_capture": "met" if real_rows else "blocked",
        "paper_proving_with_non_fixture_data": (
            "met" if proof_rows else "partial" if real_rows else "blocked"
        ),
...
def _next_commands(readiness_status: str) -> list[str]:
    if readiness_status == "RESOLUTION_LABELS_MISSING":
        return [
            (
                ...
                "--public-network-ok --run-id weather_market_manual_051"
            ),
            "Re-run after the NWS Daily Climate Report posts the official label.",
        ]
    if readiness_status in {"WEATHER_DATA_CAPTURE_BLOCKED", "MARKET_DATA_CAPTURE_BLOCKED"}:
        return [
            (
    ...
    return ["python -m quant_os.cli readiness weather-market-paper-profit-readiness"]


def _write_report(payload: dict[str, Any], *, output_root: str | Path) -> dict[str, str]:
    root = Path(output_root) / REPORT_ROOT
    root.mkdir(parents=True, exist_ok=True)
    json_path = root / "latest_weather_paper_profit_readiness.json"
    md_path = root / "latest_weather_paper_profit_readiness.md"
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    lines = [
        "# Sequence 51 Weather Paper Profit Readiness",
        "",
