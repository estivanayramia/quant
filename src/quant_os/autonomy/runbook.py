...
class RunbookEngine:
    ...
    def _merge_task_details(self, state: AutonomousRunState, name: str, result: TaskResult) -> None:
        ...
        elif name == "freqtrade_safe_lane_status":
            state.freqtrade_summary = result.details
        elif name == "dryrun_monitoring_status":
        ...
        elif name == "proving_mode_status":
            state.proving_mode_summary = result.details
        elif name == "canary_planning_status":
    ...
    def _markdown(self, state: AutonomousRunState) -> str:
        ...
        if state.freqtrade_summary:
            lines.extend(["", "## Freqtrade Dry-Run Lane", f"`{state.freqtrade_summary}`"])
        if state.dryrun_monitoring_summary:
            lines.extend(
                [
                    "",
                    "## Dry-Run Monitoring",
                    f"`{state.dryrun_monitoring_summary}`",
                ]
            )
        if state.freqtrade_trade_artifacts_summary:
            lines.extend(
                [
                    "",
        ...
        if state.dataset_evidence_summary:
            lines.extend(
                [
                    "",
                    "## Dataset Evidence",
        ...
        if state.proving_mode_summary:
            lines.extend(
                [
                    "",
                    "## Proving Mode",
                    f"`{state.proving_mode_summary}`",
                ]
            )
        if state.canary_planning_summary:
            lines.extend(
                [
                    "",
                    "## Canary Planning",
                    f"`{state.canary_planning_summary}`",
                ]
            )
        if state.canary_rehearsal_summary:
            lines.extend(
                [
                    "",
                    "## Canary Rehearsal",
